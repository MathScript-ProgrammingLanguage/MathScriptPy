	...
	if args.file:
		...
		while True:
			...
			if error: print(error.as_string())
				...
				rst = [repr(x) for x in result.elements if not getattr(x, 'hidden', False) and x is not None]
				if len(result.elements) == 1 and not getattr(result.elements[0], 'hidden', False) and result.elements[0] is not None:
